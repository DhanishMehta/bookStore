...
interface cartFunctionalityImport {
  ISBN: number;
  functionName: string;
}
...
export class MainComponent {
  ...
  @Output() isCartEmptyEmitter: EventEmitter<any> = new EventEmitter();
  ...
  filteredProducts: Array<book> = this.booksData;

  toDisplayBookID: number = 0;
  toDisplayBook: book | undefined;
  ...
  cartEmptyEvent() {
    ...
    this.isCartEmptyEmitter.emit(this.isCartEmpty);
  }

  addToCart(id: number) {
    let pdt = this.booksData.find(({ ISBN }) => ISBN == id);

    let index = this.cart.findIndex((obj) => {
      return obj.product?.ISBN === id;
    });

    if (index != -1) {
      this.cart[index].quantity++;
      this.cart[index].currPrice! += this.cart[index].product?.price?.value!;
    } else {
      let newItem = {
        product: pdt,
        quantity: 1,
        currPrice: pdt?.price.value,
      };
  ...
  removeFromCart(id: number) {
    ...
    if (index === -1) {
      alert('The item is not present in the cart!');
      return;
    }
  ...
  cartUtilityFunction(e: cartFunctionalityImport) {
    if (e.functionName === 'add') {
      this.addToCart(e.ISBN);
    } else {
  ...
  filteredSearch() {
    let filterQuery = this.searchForm.get('searchQuery')?.value;
    console.log('inside filtered search with query: ' + filterQuery);
    if (filterQuery === '') {
      this.isFilter = false;
      console.log('null filter query');
      this.filteredProducts = this.booksData;
    } else {
      this.isFilter = true;
      console.log('filter query: ' + filterQuery);
      this.filteredProducts = this.booksData.filter((book) => {
        return (
          book.title.toLowerCase().includes(filterQuery.toLowerCase()) ||
          book.summary.toLowerCase().includes(filterQuery.toLowerCase()) ||
          book.author.toLowerCase().includes(filterQuery.toLowerCase()) ||
          book.ISBN.toString().includes(filterQuery.toLowerCase())
        );
      });
    }
  }
  ...
  changeDisplay() {
    if (this.toDisplayBook === undefined) {
  ...
  displayCardDetails(e: number) {
    console.log('caught book isbn in main');
    this.toDisplayBookID = e;
    this.toDisplayBook = this.booksData?.find(
      (obj) => obj.ISBN === this.toDisplayBookID
    );
    this.changeDisplay();
  }
}
